...
class UserFragment : Fragment() {
    ...
    inner class UserFragmentRecyclerViewAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
        var contendDTOs: ArrayList<ContentDTO> = arrayListOf()
            ...
            firestore?.collection("images")?.whereEqualTo("uid", uid)
                ?.addSnapshotListener { querySnapshot, firebaseFirestoreException ->
                    ...
                    for (snapshot in querySnapshot.documents) {
                        contendDTOs.add(snapshot.toObject(ContentDTO::class.java)!!)
                    }
                    fragmentView?.account_tv_post_count?.text = contendDTOs.size.toString()
        ...
        override fun getItemCount(): Int {
            return contendDTOs.size
        }

        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            var imageview = (holder as CustomViewHolder).imageview
            Glide.with(holder.itemView.context).load(contendDTOs[position].imageUrl)
